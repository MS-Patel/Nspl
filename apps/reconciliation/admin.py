...
class RTAFileAdmin(admin.ModelAdmin):
    list_display = ('rta_type', 'file_name', 'status', 'uploaded_at', 'processed_at')
    search_fields = ('file_name', 'rta_type')
    list_filter = ('rta_type', 'status', 'uploaded_at')

@admin.register(Transaction)
class TransactionAdmin(admin.ModelAdmin):
    list_display = ('investor', 'scheme', 'folio_number', 'txn_type_code', 'amount', 'date', 'txn_number')
...
class HoldingAdmin(admin.ModelAdmin):
    list_display = ('investor', 'scheme', 'folio_number', 'units', 'current_value', 'last_updated')
