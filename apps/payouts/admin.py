...
class DistributorCategoryAdmin(admin.ModelAdmin):
    list_display = ('id', 'name', 'min_aum', 'max_aum', 'share_percentage')
    search_fields = ('name',)

@admin.register(BrokerageImport)
class BrokerageImportAdmin(admin.ModelAdmin):
    list_display = ('id', 'month', 'year', 'status', 'uploaded_at')
    list_filter = ('status', 'year')
    search_fields = ('year', 'month')

@admin.register(BrokerageTransaction)
class BrokerageTransactionAdmin(admin.ModelAdmin):
    list_display = ('id', 'source', 'transaction_date', 'investor_name', 'scheme_name', 'amount', 'brokerage_amount', 'distributor', 'is_mapped')
...
class PayoutAdmin(admin.ModelAdmin):
    list_display = ('id', 'distributor', 'total_aum', 'category', 'gross_brokerage', 'share_percentage', 'payable_amount', 'status')
