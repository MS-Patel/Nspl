...
import { Users, IndianRupee, Repeat, ArrowUpRight } from 'lucide-react';
...
const DistributorDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await api.get('/api/dashboard/distributor/');
        setData(result);
      } catch (error) {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.active_sip_count}</div>
            <p className="text-xs text-muted-foreground">Recurring Investments</p>
          </CardContent>
        </Card>
      </div>
